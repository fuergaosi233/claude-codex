...
| Subagent (Task) | Supported | A Task tool_use opens a subagent context; nested text/tool_use/tool_result events are hidden so Codex App renders one Agent item that completes with the subagent's final result, instead of the wall of leaked sub-actions a naive forward would produce. |
