...
| Subagent (Task) | Supported (native) | Claude's Task tool spawns an ephemeral child thread (threadSource=subagent) and emits a Codex `collabAgentToolCall` item in the parent (tool=spawnAgent, receiverThreadIds=[childId]) instead of an mcpToolCall. Inner text/tool_use/tool_result events are hidden from the parent; the subagent's final result is materialized as a single agentMessage on the child thread so the user can drill in from the Agent item. |
